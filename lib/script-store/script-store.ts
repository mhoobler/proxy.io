...
export function AddScript(path: string, cb: ProxyFunction) {
  Memo = {};
...
export function SearchScript(url: string | void) {
  ...
  for (const str of split) {
    ...
    switch (switchCase) {
      case 1: {
        // We're on an object, and need to explore it
      ...
      case 2: {
        // We're ending on a function/undefined
        const result = curr[str] || lastStar;
        Memo[url] = result;
        return result as ProxyFunction;
      }
      case 3: {
        // We're ending on an object, but path ends
      ...
      case 0: {
        // We're on a function/undefined, but path continues
        Memo[url] = lastStar;
        return lastStar;
      }
    }
